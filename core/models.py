...
class Compañia(models.Model):
    ...
    personajes= models.ManyToManyField(Personaje) #muchos a muchos con Personaje
...
class Figura(models.Model):
    ...
    personaje= models.ForeignKey(Personaje, on_delete= models.CASCADE) #me refiero a la tabla Personaje - el on_delete models CASCADE me rompe todo porque si borro Personaje estoy mirando a alg oque no existe si es que lo borro. Entonces on cascade si borro un personaje me lleov puesto todas las figuras que estan asociadas a ese personaje (deberia plantearse al reves la relacion pero no hay tiempo)
    coleccion= models.CharField(max_length=40, verbose_name='Colección/serie')
    compañia= models.ForeignKey(Compañia, on_delete= models.CASCADE)
